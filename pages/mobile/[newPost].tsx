...
const New: NextPageWithLayout = () => {
    ...
    useEffect(()=>{
      ...
      const handleGetDoc = async () => {
        const falseData = await mobile.filter(data => data.title === newPost)
...
export const getStaticPaths = async ({ locales }:{locales:Array<string>}) => {
  const data = await getDocs(query(collection(db,"news"),where("category","==","mobile"))); 
