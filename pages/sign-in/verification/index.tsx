  ...
  const resedEmailVerification = () => {
    if (auth.currentUser) {
      sendEmailVerification(auth.currentUser)
        ...
        .catch((err) => {
          console.log(
            "🚀 ~ file: index.tsx ~ line 25 ~ sendEmailVerification ~ err.message",
            err.message
          );
        });
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      currentUser
        ...
        .then(() => {
          if (currentUser?.emailVerified) {
            ...
            signOut(auth)
              ...
              .catch((err) => {
                console.log(
                  "🚀 ~ file: index.tsx ~ line 54 ~ signOut ~ err",
                  err.message
                );
              });
          }
        ...
        .catch((err) => {
          console.log(
            "🚀 ~ file: index.tsx ~ line 60 ~ currentUser?.reload ~ err.message",
            err.message
          );
        });
    }, 1000);

    if (!currentUser && !timeActive) {
      Router.push("/sign-in");
      setRender(false);
    } else {
      setRender(true);
      ...
      {render && (
        <div className=" bg-DarkBlueGray flex flex-col items-center space-y-5 mx-auto w-[21rem] -rotate-1 rounded-2xl py-10 px-6 mt-24  md:w-[30rem] xl:mt-36">
          ...
          <button
            ...
            } bg-Lavender-Blue rounded-xl mt-6 p-2 -rotate-12 text-Blue-Gray lg:hover:opacity-50 active:bg-white cursor-pointer`}
            onClick={() => {
              setAnimation(true);
          ...
          >
            <p>
              ...
              <span
                className={`${
                  timeActive &&
                  "ml-2 text-white  bg-DarkBlueGray rounded-md p-1"
                }`}
              >
                {timeActive && time}
              </span>
