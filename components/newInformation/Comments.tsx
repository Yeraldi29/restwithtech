...
const Comments = ({
  ...
  const [parentComments, setParentComments] =
  ...
  useEffect(() => {
    const commentsDoc = async () => {
      setLoading(true);
      const getCommentsLenght = await (
        await getDocs(collection(db, `comments`, `${idNewPost}`, `comment`))
      ).docs.length;
      const getParentComment = await getDocs(
        query(
    ...
    <>
      <div className="relative mt-8 mb-4 flex space-x-1">
        ...
        <div className="w-14 h-14 xl:w-16 xl:h-16  -mt-4 sm:-mt-1 relative flex items-center justify-center">
          ...
          <span className="text-DarkBlueGray text-lg xl:text-xl rotate-12 font-semibold">
            {commentsLenght}
          </span>
        </div>
      </div>
