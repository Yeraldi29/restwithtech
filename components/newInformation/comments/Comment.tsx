...
const Comment = ({
  ...
  useEffect(() => {
    const fetchLenght = async () => {
      const commentRepliesLength = (
        await getDocs(
          query(
            collection(db, `comments`, `${idNewPost}`, `comment`),
  ...
  return (
    <>
      <div
        className={` mt-3 ${parent >= 1 ? "ml-8 " : "lg:mx-0"} break-words `}
      >
        <HeaderComm
          username={username}
        ...
        <FooterComm
          ...
          idNewPost={idNewPost}
          parent_id={parent_id}
          commentRepliesLength={commentRepliesLength}
        ...
        {commentRepliesLength > 0 && (
          <div
            className={`${
              !showReplies
                ? "bg-BabyBlueEyes text-BlueDarker hover:bg-DarkBlueGray hover:text-white "
                : " bg-DarkBlueGray text-white hover:bg-BabyBlueEyes hover:text-BlueDarker hover:border-white"
            } 
          w-fit mt-2 border-4 border-Blue-Gray rounded-lg rotate-1 p-1  cursor-pointer transform ease-in-out duration-500 md:text-lg xl:text-xl md:p-2`}
      ...
      {replyComment && (
        <div
          className={`relative mt-4 lg:h-fit ${
            parent >= 1 ? "ml-8 " : "lg:mx-0"
          }`}
        >
          ...
          <CreateParagraph
            ...
            name={name}
            parent_id={parent_id}
            dataFather={serialize(JSON.parse(data))}
      ...
      {showReplies && !commentReplies?.empty && (
        <div className="relative">
          {commentReplies?.docs
            .filter((doc) => doc.data().parent_id === parent_id)
            .map((doc, index) => (
              <Comment
                key={index}
                idNewPost={idNewPost}
                name={name}
                parent_id={doc.data().id}
                parent={doc.data().parent_id}
                data={doc.data().data}
                username={doc.data().username}
                imageProfile={doc.data().imageProfile}
                create_at={doc.data().create_at}
                dataFather={doc.data().replyComment}
