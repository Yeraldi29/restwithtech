  ...
  return (
    <div
      className={`relative p-2 ${
        seeMore ? "h-auto pb-10" : "max-h-64 "
      } overflow-hidden bg-Lavender-Blue/40 text-BlueDarker rounded-xl border-4 ${
    ...
    >
      <div ref={refData}>
        {parent > 0 && (
          <div className="w-full bg-BabyBlueEyes/60 p-1 mb-2 text-sm rounded-md border-2 border-DarkBlueGray">
            <h4 className="text-base text-BlueDarker font-bold ">
              {replyUsername}
            </h4>
            <p>
              {dataFather} <strong> ...</strong>
            </p>
          </div>
        )}
      ...
      {refData.current && refData.current?.offsetHeight > 256 && (
        <div
          className={`absolute bottom-1 right-1 flex items-center space-x-1 p-1 rounded-md border-2 text-white cursor-pointer transform duration-200 ease-out 
           ${
             username === currentUser?.displayName
               ? "border-DarkBlueGray bg-Blue-Gray lg:hover:text-DarkBlueGray"
