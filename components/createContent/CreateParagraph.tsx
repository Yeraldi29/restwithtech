...
const CreateParagraph = ({
  ...
  const { setContentComment, handleSaveComment, saved } = useCreateComment(
    idNewPost,
    name,
    parent_id,
    dataFather,
  ...
  return (
    <>
      <div
        className={`w-full h-fit ${
          saved === "wait" || savedParagraph === "wait"
            ? "bg-gray-400"
            : "bg-Lavender-Blue/40"
        }  rounded-xl border-4 border-Blue-Gray text-BlueDarker md:text-lg xl:text-xl  focus:outline-none ${
          grow ? "min-h-[5rem] " : "min-h-[3rem]"
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-10 w-full relative mt-2 px-2 pb-1 sm:grid-cols-14 lg:grid-cols-16">
            ...
            {plainText.length >= 1 && (
              <div
                className={`relative sm:col-span-1 cursor-pointer ${
                  !grow
                    ? "max-h-0 opacity-0"
                    : "opacity-100 transform duration-500 ease-in"
                }`}
                onClick={handleSendContent}
              >
                <BiSend
                  className={`absolute -bottom-[0.05rem] ml-1 w-7 h-7 xl:w-9 xl:h-9 -rotate-12 lg:hover:text-white lg:hover:rotate-12   text-DarkBlueGray ${
                    !grow
                      ? "max-h-0 opacity-0"
                      : "opacity-100 transform duration-500 ease-in"
      ...
      {plainText.length >= 1 && <CannotSave text={t("changes")} />}
      {plainText.length <= 20 && clickSend && (
        <CannotSave text={t("cannotSave")} />
      )}
      {(saved === "wait" || savedParagraph === "wait") && <Loading />}
    </>
  );
};
...
const Leaf = ({ attributes, children, leaf }: RenderLeafProps) => {
  ...
  return (
    <span
      className={`${leaf.text === "" ? " pl-[0.1px]" : null}`}
      {...attributes}
    >
      {children}
    </span>
