...
const PostNew = () => {
  ...
  return (
    ...
    onMouseOver={() => handleNewExpand(true)} onMouseLeave={() => handleNewExpand(false)}>
        <BiPlus className=" w-9 h-9 lg:group-hover:scale-125 duration-200 ease-in"/>
        <h2 className={`pr-1 ${NewExpand ? " opacity-100" : " opacity-0 absolute -right-6"} duration-300 ease-in-out transition`}>{t("postNew").toUpperCase()}</h2>
    </div>
  )
}
