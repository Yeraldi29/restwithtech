  ...
  return (
    ...
    <div className="hidden relative lg:flex items-center text-center rounded-xl -rotate-12">
        ...
        onAnimationEnd={()=>setAnimation(false)}>
          <h2 className={`${click ? "text-red-400 ":  "text-white "}  text-md xl:text-lg group-hover:font-extrabold `}>
            {t("account")}
          </h2>
        </button>
        <div className={`transition duration-500 ease-in absolute top-24 bg-DarkBlueGray border-4 border-Blue-Gray w-32 h-28 flex items-center justify-center rounded-2xl right-10 ${click ? "z-50": "opacity-0 -z-10 -translate-y-60"}`}>
          <UserAccount />
          <Bubbles click={click}/>
        </div>
    </div>
