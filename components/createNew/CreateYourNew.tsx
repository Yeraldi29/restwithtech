    ...
    const handleGetValuesCreateNew = async () => {
      if(currentUser?.uid){
        ...
        if(docsUserCreateNew.empty){
          ...
          const docCreateNew = await getDocs(query(collection(db, "users", currentUser.uid, "userCreateNew"),where("creating", "==", true)))
      ...
      <div className="md:text-lg col-span-3">
        {!published && (
          ...
          {loadingContent ? (
            ...
            <>
            <MainTitle getDocumentName={getDocumentName} getDocValues={getDocValues} previewContent={previewContent} />
            <ChooseCategory getDocumentName={getDocumentName} getDocValues={getDocValues} previewContent={previewContent} />
            <MainImage getDocumentName={getDocumentName} getDocValues={getDocValues} previewContent={previewContent} />
            {!getContentBody?.empty && (
              <div className={`${previewContent && "border-4 border-DarkBlueGray mt-4 rounded-xl py-4 xl:text-xl"}`}>
              {getContentBody?.docs.map((doc, index)=>(
                <ContentBody key={index} dataParagraph={doc.data().data} dataImage={doc.data().image} option={doc.data().option} order={doc.data().order} getDocumentName={getDocumentName} previewContent={previewContent} />
              ))}
              </div>
            )}
            { addCreateParagraph && (
              ...
              <div className=" w-full flex justify-end items-center">
                <div className=" mt-4 w-fit p-1 bg-red-500 rounded-xl text-white border-4 border-Blue-Gray cancelOrDelete " onClick={()=>handleClickAddCreateParagraph(false)}>
                  <h3 className="text-xl">{t("cancel")}</h3>
                </div> 
              </div>
              </div>
            )}
            <CreateNewOptions getDocumentName={getDocumentName} getDocValues={getDocValues} handleClickAddCreateParagraph={handleClickAddCreateParagraph} handleClickPreviewContent={handleClickPreviewContent} previewContent={previewContent} />
            </>
          )}
          </>
        )}
        {(!previewContent && !loadingContent) && (
            <PublicNew getDocValues={getDocValues} getDocumentName={getDocumentName} getContentBody={getContentBody} published={published} handlePublished={handlePublished} />
        )}
      </div>
