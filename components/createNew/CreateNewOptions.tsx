...
const CreateNewOptions = ({
  getDocumentName,
  handleClickAddCreateParagraph,
  handleClickPreviewContent,
  previewContent,
}: createNewOptionsProps) => {
  ...
  const handleClickAddImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      if (currentUser?.uid) {
        ...
        const getDocsContentBodyLength = (
          await getDocs(
            collection(
              db,
              "users",
        ...
        const uploadMainImage = uploadString(
          ref(
            storage,
            `users/${currentUser?.uid}/userCreateNew/${getDocumentName}/${
              getDocsContentBodyLength + 1
            }`
          ),
  ...
  return (
    <>
      <div className="mt-8 flex items-center space-x-4">
        <motion.div
          className={`flex items-center border-4 border-Blue-Gray rounded-xl overflow-hidden lg:hover:bg-white lg:hover:text-Blue-Gray lg:hover:border-BlueDarker cursor-pointer transform duration-300 ease-in-out`}
          onClick={() => setAnimation({ ...animation, plus: !animation.plus })}
          animate={
            animation.plus
              ? { rotate: 0, backgroundColor: "white" }
              : { rotate: -12, backgroundColor: "transparent" }
          }
        >
          <motion.div
            animate={
              animation.plus
                ? { translateX: -40, display: "none" }
                : { translateX: 0, display: "block" }
            }
          >
          ...
          >
            <div
              className=" border-r-4 p-[2px] border-Blue-Gray lg:hover:bg-BlueDarker lg:hover:text-white cursor-pointer transform duration-300 ease-in-out"
              onClick={handleClickAddParagraph}
            >
              <BiParagraph className="rotate-12 w-12 h-12" />
            </div>
            <div
              className="p-[2px] lg:hover:bg-BlueDarker lg:hover:text-white cursor-pointer transform duration-300 ease-in-out"
              onClick={() => inputFileRef.current?.click()}
            >
              <BiImageAdd className="-rotate-12 w-12 h-12" />
              <input
                onChange={handleClickAddImage}
                ref={inputFileRef}
                type="file"
                name="file"
                hidden
        ...
        {previewContent ? (
          <div
            className={`rotate-12 border-4 border-Blue-Gray rounded-xl lg:hover:text-Blue-Gray lg:hover:border-BlueDarker cursor-pointer transform duration-300 ease-in-out ${
              animation.show && " animate-wiggle"
            }`}
        ...
        <div
          className={`-rotate-12 border-4 border-Blue-Gray bg-red-400 text-white rounded-xl cancelOrDelete ${
            animation.help && " animate-wiggle"
          }`}
