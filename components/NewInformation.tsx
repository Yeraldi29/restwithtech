    ...
    <>
        <div className=" lg:grid lg:grid-cols-5 gap-x-8">
          <div className=" text-BlueDarker md:text-lg col-span-3">
            ...
            {t("published")} 
                {option === "fakeData" ? (
                 <span className=" text-Blue-Gray font-semibold"> {t("hour",{timeFake})}</span>
                ): option === "data" && (
                  <span className=" text-Blue-Gray font-semibold"> {timeString}</span>
                )} 
                 <span> {t("by")} </span> 
                {option === "fakeData" ? (
                     ...
                     {data.data().option === "paragraph" ? (
                      ...
                      <div className="my-4 col-span-3 relative bg-DarkBlueGray w-full h-[22rem] sm:h-[27rem] md:h-[30rem] lg:h-[28rem] xl:h-[32rem] rounded-xl rotate-1  border-4 border-DarkBlueGray break-words">
                        <div className="relative w-full h-full">
                            <Image className="rounded-lg" src={data.data().data} alt={"image content"} fill />
                        </div>
                      </div>
                     )}
                    </>
        ...
        {news?.docs.map((data,index) => {
            if(data.data().mainTitle !== title){
              return <ItemPost image={data.data().mainImage} category={data.data().category} time={data.data().create_at} name={data.data().mainTitle} option="data" key={index} index={index} title={data.data().mainTitle}/>
            }
        }).concat(mostRecents.map((post,index) => {
            if(post.title !== title){
            return <ItemPost image={post.image  } category={post.category} timeFake={post.time} name={post.name} option="fakeData" key={post.name} index={index} title={post.title} />
            }
          }
          ))
        }
