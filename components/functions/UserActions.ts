        ...
        createUserWithEmailAndPassword(
          ...
          .then((userCredentials) => {
            sendEmailVerification(userCredentials.user).then(() => {
              Router.push(
                `${locale === "es/" ? locale : ""} sign-in/verification`
              );
          ...
          .catch((err) => {
            ...
            console.log(
              "🚀 ~ file: FormInputs.tsx ~ line 64 ~ createUserWithEmailAndPassword ~ err.message",
              err.message
            );
          });
        break;
      ...
      case t("logIn.login"): {
        signInWithEmailAndPassword(auth, formValues.email, formValues.password)
          .then((userCredantials) => {
            ...
            if (user.emailVerified) {
              if (user.displayName === null && user.photoURL === null) {
                Router.push(
                  `${locale === "es/" ? locale : ""}log-in/completeProfile`
                );
              } else {
                Router.push("/");
              }
          ...
          .catch((err) => {
            handleOther(err.message);
            console.log(
              "🚀 ~ file: UserActions.ts ~ line 38 ~ UserActions ~ err.message",
              err.message
            );
          });
        break;
