...
const EditProfile = ({
  ...
  const { handleDone, handleEditProfile, editProfile } =
  ...
  return (
    <div className="lg:w-[38rem] xl:w-[52rem] lg:flex lg:justify-center ">
      <div className="border-4 border-DarkBlueGray text-BlueDarker rounded-xl mt-4 p-2 sm:max-w-md sm:mx-auto lg:m-0 ">
        ...
        <Biography
          handleProfileValues={handleProfileValues}
          descriptionError={descriptionError}
          descriptionValue={profileValues.description}
        />
        <Skills
          handleProfileValues={handleProfileValues}
          skill1Error={skill1Error}
          skill2Error={skill2Error}
          skill3Error={skill3Error}
          skill1Value={profileValues.skill1}
          skill2Value={profileValues.skill2}
          skill3Value={profileValues.skill3}
        />
        <Profession
          handleProfileValues={handleProfileValues}
          professionError={professionError}
          professionValue={profileValues.profession}
        />
        <div>
          <div
            className={`w-fit mx-auto my-6 p-2  border-4 ${
              saveError
                ? "border-red-500 bg-red-200 hover:bg-red-500 hover:border-DarkBlueGray"
                : " border-DarkBlueGray bg-Lavender-Blue hover:bg-DarkBlueGray hover:text-white hover:border-Blue-Gray"
            } ${
              animation && "animate-wiggle"
            } rounded-xl cursor-pointer transform ease-out duration-300`}
          ...
          {loading && <Loading />}
          {saveError && (
            <h3 className="text-red-500 text-lg text-center -mt-4">
              {t("warningSave")}
            </h3>
          )}
          {editProfile && !loading && (
            <div
              className="w-fit mx-auto my-6 p-2  border-4 border-red-500 bg-red-200 hover:bg-red-500 hover:border-DarkBlueGray rounded-xl cursor-pointer transform ease-out duration-300"
