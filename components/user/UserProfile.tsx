...
const UserProfile = ({descriptionProfile}:{descriptionProfile:string}) => {
  const [ imageUser, setImageUser ] = useState("")
  const [ loading, setLoading ] = useState(false)
  const [ updateImage, setUpdateImage ] = useState(false)
  
  const { currentUser } = useAuthValue()
  const { editProfile, handleEditProfile, handleChangeImage } = useUserProfileContent()
  ...
  return (
    <div className=" border-4 border-DarkBlueGray text-BlueDarker rounded-xl mt-4 p-2 sm:max-w-md sm:mx-auto lg:sticky top-24 lg:m-0 lg:h-fit lg:max-w-none lg:w-96">
     <div className="flex items-center justify-center lg:flex-col space-x-4 rounded-xl p-2 py-4 ">
      ...
      {descriptionProfile !== "" && (
        <>
        {editProfile && (
        <div className={`w-fit mx-auto -mb-4${loading ? "-mt-2" : " -mt-8"}`}>
          <UploadImage handleUpdateImage={handleUpdateImage} message={t("changeImage")}/>
        </div>
        )}
        <div className={`text-white xl:text-lg bg-DarkBlueGray mb-2 ${!editProfile && "mt-4"} p-2 border-4 border-Blue-Gray rounded-xl`}>
          <p>{descriptionProfile}</p>
        </div>
        {!editProfile && (
          <div className="w-fit mx-auto mb-3 p-2 bg-DarkBlueGray border-4 border-Blue-Gray rounded-xl -rotate-1 text-xl xl:text-2xl text-white cursor-pointer hover:bg-Lavender-Blue hover:text-DarkBlueGray transform ease-out duration-300"
